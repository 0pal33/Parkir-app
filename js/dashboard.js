...
function rupiah(n){
  return window.dashboardShared.formatRupiah(n)
}
...
async function loadDashboard(){
  ...
  let { data: stokBarang } = await window.supabaseClient
  .from("stok_barang")
  .select(`
    id,
    nama_item,
  ...
  let { data: barangTitipan } = await window.supabaseClient
  .from("barang_titipan")
  .select(`
    id,
    nama_item,
  ...
  // BULANAN
  ;(bulanan ?? []).forEach(b=>{
    let val = window.dashboardShared.calcBulananIncome(b)
    income += val
    bulananTotal += val
    bulananCount++
  })
